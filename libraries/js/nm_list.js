...
var userNm="";//사용자 이름
var userAge=0;//사용자 나이
var userHeight=0;//사용자 키
var userWeight=0;//사용자 체중
var userSex=""; //성별 남1 여2
var userLifestyle="";//라이프스타일
var userPurpose=""; //운동목적 
var userTypeCarb=0;//탄수화물
var userTypeProt=0;//단백질
var userTypePat=0;//지방
 ...
 */
	$(function(){
		...
		if(userNm!=null){
			...
			userLifestyle=sessionStorage.getItem('userLifestyle');
			...
			var userLifestyleNum=0;

			switch (userLifestyle){
				case 'verLight' :
					userLifestyleNum=1.3;
					break;
				case 'light' :
					userLifestyleNum=1.55;
					break;
				case 'moderate' :
					userLifestyleNum=1.65;
					break;
				case 'heavy' :
					userLifestyleNum=1.80;
					break;
				case 'veryHeavy' :
					userLifestyleNum=2.00;
					break;
			}

			var sBmr=bmr*userLifestyleNum;//권장대사량
			...
			switch (userPurpose){
				...
				case 'muscleUp' :
					...
					break;
				case 'typeValue' :
					userTypeCarb=sessionStorage.getItem('userTypeCarb');
					userTypeProt=sessionStorage.getItem('userTypeProt');
					userTypePat=sessionStorage.getItem('userTypePat');
					sBmr+=0;
					break;
			}
			...
			//임시적으로 막음 0518
			// alert('로그인해주세요.');
			// location.replace('/nutritionChk/nutritionMon/member');
			// return;
		}
		 ...
		 page2_list_dt=$("#foodResTbl").DataTable({
			...
			* filtered data 총합만 계산하도록 함.*/
			,"footerCallback":function(){
				...
				$('#idCol').text(parseInt(result5)); //inputBox 열량
				$('#idColbo').text(parseInt(result6)); //inputBox 탄수화물
				$('#idProt').text(parseInt(result7)); //inputBox 단백질
				$('#idFat').text(parseInt(result8)); //inputBox 지방		
			}
		 }); 
	});
	...
	*/
	 ...
	 * 결과값이 바뀌었을때 이미지 변경
	 */
	$("#idColbo,#idProt,#idFat,#idCol").on('DOMSubtreeModified',function(){
